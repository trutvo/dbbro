...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Path to the YAML connections file, holding the 'connections' "
            "mapping and optional 'default' alias. Kept separate from "
            "--config so connection credentials don't have to live in the "
            "(potentially shared/committed) schema config."
        ),
    )
    parser.add_argument(
        "--connection",
        help=(
            "Alias of the connection to use, from the --connections file's "
            "'connections' mapping. Defaults to that file's 'default' alias, "
            "or the sole connection if only one is declared."
        ),
    )
    parser.add_argument(
        "--search",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        connections_raw = read_yaml_file(args.connections)
    except ConfigValidationError as exc:
        return _report_and_exit(exc)

    try:
        db_config = resolve_database_config(
            connections_raw, env=os.environ, connection=args.connection
        )
