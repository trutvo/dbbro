...
def draw_modal(screen, lines: list[str], highlighted_index: int | None = None) -> None:
    ...
    line (truncated to the terminal's current width), reverse-videoing the
    line at `highlighted_index` if given."""
    ...
    c = MODAL_CHARS
    _write_line(screen, 0, 0, c["tl"] + c["h"] * (box_width - 2) + c["tr"])
    for i, line in enumerate(lines):
        ...
        if row >= max_height - 1:
            break
        _write_line(screen, row, 0, c["v"] + " " + content.ljust(inner_width) + " " + c["v"], attr)
    last_row = min(box_height - 1, max_height - 1)
    _write_line(screen, last_row, 0, c["bl"] + c["h"] * (box_width - 2) + c["br"])
